  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      <div className="flex flex-col sm:flex-row justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-[var(--rose-900)]">Gestión de Usuarios</h2>
          <p className="text-[var(--rose-600)]">Administrá quién tiene acceso al panel de control.</p>
        </div>
      </div>
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-[var(--rose-100)]">
              {isLoading ? (
                ...
                users.map((user) => (
                  <tr key={user.id} className="hover:bg-[var(--rose-50)/50] transition-colors">
                    <td className="p-4 font-medium text-[var(--rose-900)] flex items-center gap-3">
