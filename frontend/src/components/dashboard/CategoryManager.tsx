        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-[var(--rose-100)]">
              {isLoading ? (
                <tr>
                  <td colSpan={2} className="p-8 text-center text-[var(--rose-500)] animate-pulse">
                    Cargando categorías...
                  </td>
                </tr>
              ) : filteredCategories.length === 0 ? (
                <tr>
                  <td colSpan={2} className="p-8 text-center text-[var(--rose-500)]">
                    No se encontraron categorías.
                  </td>
                </tr>
              ) : (
                filteredCategories.map((category) => (
                  <tr key={category.id} className="hover:bg-[var(--rose-50)/50] transition-colors">
                    <td className="p-4 font-bold text-[var(--rose-900)]">{category.name}</td>
      ...
      >
        <form key={editingCategory ? editingCategory.id : 'new'} className="space-y-4" onSubmit={handleSubmit}>
          ...
          <div>
            ...
            <Input name="name" type="text" defaultValue={editingCategory?.name} placeholder="Ej: Manicura y Esmaltado" required />
          </div>
