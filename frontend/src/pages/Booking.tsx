...
export default function Booking() {
    ...
    const fetchServicesAndAddons = async () => {
      try {
        const [servRes, addRes] = await Promise.all([
          ...
          fetch("https://abrunails.onrender.com/api/addons")
        ]);
  ...
  useEffect(() => {
    if (selectedDate && totalDuration > 0) {
      const fetchAvailability = async () => {
        ...
        try {
          const response = await fetch(`https://abrunails.onrender.com/api/availability?date=${selectedDate}&duration=${totalDuration}`);
          if (response.ok) {
  ...
  const handleWhatsAppBooking = async () => {
    ...
    try {
      const clientRes = await fetch("https://abrunails.onrender.com/api/clients", {
        method: "POST",
      ...
      const aptRes = await fetch("https://abrunails.onrender.com/api/appointments", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
